# EV-Charging-Station-Utilization-Analysis
This repository contains the complete analysis of EV Charging Station Utilization based on real-time station usage logs. The project includes data preprocessing, analysis, visualizations, insights, and recommendations.


---

## 📌 Project Overview

This project analyzes the utilization patterns of EV charging stations. It focuses on:

* Identifying high-demand hours
* Monitoring queue lengths
* Calculating session durations
* Spotting overloaded stations
* Recommending new charging point locations

---

## 🔧 Technologies Used

* **Python**
* **Pandas** for data processing
* **Matplotlib / Seaborn** for charts
* **Jupyter Notebook** for analysis

---

## 📊 Key Insights

* Hour-wise usage trends
* Station-wise session count
* Queue analysis
* Peak demand periods
* Recommended stations for capacity expansion

---

## 📥 How to Use This Repository

1. Clone the repo:

```
git clone https://github.com/your-username/EV-Charging-Station-Utilization-Analysis.git
```

2. Open the Jupyter Notebook.
3. Install required dependencies.
4. Run the analysis.

---

## 📎 Uploading on GitHub

🔗 **Notebook File Link:**
[https://github.com/Rajnandini-0804/EV-Charging-Station-Utilization-Analysis/blob/main/EV%20Charging%20Station%20Utilization%20Analysis%20(1).ipynb](https://github.com/Rajnandini-0804/EV-Charging-Station-Utilization-Analysis/blob/main/EV%20Charging%20Station%20Utilization%20Analysis%20%281%29.ipynb)

🔗 **Dataset Link:**
[https://github.com/Rajnandini-0804/EV-Charging-Station-Utilization-Analysis/blob/main/ev_charging_station_dataset.csv](https://github.com/Rajnandini-0804/EV-Charging-Station-Utilization-Analysis/blob/main/ev_charging_station_dataset.csv):**
[https://github.com/Rajnandini-0804/EV-Charging-Station-Utilization-Analysis/blob/main/EV%20Charging%20Station%20Utilization%20Analysis%20(1).ipynb](https://github.com/Rajnandini-0804/EV-Charging-Station-Utilization-Analysis/blob/main/EV%20Charging%20Station%20Utilization%20Analysis%20%281%29.ipynb)


---

## ✨ Author

**Rajnandini Bhosale**


